...
cenarios = {
    "Cenário 1 (Normal)": dados,
    "Cenário 2 (Atendimentos mais rápidos)": [(chegada, servico // 2) for chegada, servico in dados],
    "Cenário 3 (Chegada mais espaçada)": [(chegada * 1.5, servico) for chegada, servico in dados],
    "Cenário 4 (Chegada mais intensa)": [(chegada * 0.5, servico) for chegada, servico in dados],
    "Cenário 5 (Número reduzido de atendentes)": dados[:50]  # Simulando menos clientes
}

# Simulação dos cenários
resultados = {}
for nome_cenario, dados_cenario in cenarios.items():
    tempo_medio = simulador_atendimento(dados_cenario)
    resultados[nome_cenario] = tempo_medio

# Exibindo os resultados
for cenario, tempo in resultados.items():
    print(f"{cenario}: Tempo médio de espera = {tempo:.2f} unidades de tempo")
